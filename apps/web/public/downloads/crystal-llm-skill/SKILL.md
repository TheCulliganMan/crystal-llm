...
- Use the non-wandering route method whenever the user corrects aimless movement, a dungeon has multiple floors, a route has been revisited, or the next landmark is not immediately reached. Before moving, write a compact route ledger in the working notes or user update: `target`, `current`, `visitedEdges`, `deadEdges`, `frontier`, `lastLandmark`, and `nextSegment`. Pick exactly one target landmark or warp from live `route`; if the target is not known, pick the nearest unvisited frontier that can reveal a new landmark. Move only 3-6 tiles or to the next junction/warp/trainer, then call `poke observe` or `poke status` and update the ledger. A segment counts as progress only if it reaches the named landmark, discovers a new branch/warp/objective, marks a specific edge dead from live evidence, or changes floors/maps. Repeating a coordinate, reversing the last segment, or drifting to an unrelated hotspot is not progress; stop, call `poke route --tiles`, and choose a different frontier.
- For floor-change requests, make the floor-changing warp/stairs/hole the target immediately. Do not detour to items, trainers, shelves, signs, or broad story labels unless the route to the floor change is blocked by that exact object. If multiple warps are visible, choose the one whose live target changes to the requested floor; if the warp is not reachable from the current connected component, route to the nearest connector that could reach it and record the current component as unresolved rather than wandering.
- Keep visited path memory local and concrete. Record coordinates and edge outcomes such as `(33,19)->west corridor reached (21,19)`, `(17,27)->west blocked`, or `warp (7,29)->B2F`. Do not store blind button strings as the route. When leaving a floor, preserve the floor entry coordinate, the warp used, and the unexplored frontiers left behind so future runs can continue deliberately instead of rediscovering the same pockets.
