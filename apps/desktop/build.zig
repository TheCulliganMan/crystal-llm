...
pub fn build(b: *std.Build) void {
    ...
    const frontend_install = b.addSystemCommand(&.{ "npm", "--version" });
    ...
    const frontend_build = b.addSystemCommand(&.{ "npm", "--version" });
    ...
    const package = b.addSystemCommand(&.{
        ...
        optimize_name,
        "--output",
        b.fmt("dist/{s}-1.0.0-{s}-{s}{s}", .{ app_exe_name, @tagName(package_target), optimize_name, packageSuffix(package_target) }),
        "--binary",
    });
