    ...
    fn heal_party_preserves_unknown_move_pp_and_heals_known_fields() {
        ...
        let healed = state.storage.party.pokemon[0].as_ref().expect("party Pokemon");
    ...
    fn give_odd_egg_uses_exact_table_entry_and_modpack_move_pp() {
        ...
        assert_eq!(
            egg.moves,
            vec![
                LearnedMove { name: "POUND".to_string(), current_pp: 35, pp_ups: 0 },
                LearnedMove { name: "CHARM".to_string(), current_pp: 20, pp_ups: 0 },
                LearnedMove { name: "DIZZY_PUNCH".to_string(), current_pp: 10, pp_ups: 0 },
            ]
        );
        assert_eq!(
            state
