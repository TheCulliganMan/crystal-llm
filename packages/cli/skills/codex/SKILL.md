...
## Running a Task
1. Default to `gpt-5.5` model. Do not downgrade this default. Ask the user (via `AskUserQuestion`) which reasoning effort to use (`xhigh`,`high`, `medium`, or `low`). User can override model if needed (see Model Options below). Do not check the CLI for a model list; use this model table as the source of truth.
...
| --- | --- | --- | --- |
| `gpt-5.5` ⭐ | **Default model**: Frontier software engineering, agentic coding workflows | 400K input / 128K output | Use as the default for Codex unless Ryan explicitly overrides it |
| `gpt-5.2-max` | **Max model**: Ultra-complex reasoning, deep problem analysis | 400K input / 128K output | Legacy fallback only if explicitly requested |
| `gpt-5.2` | **Legacy flagship model**: Software engineering, agentic coding workflows | 400K input / 128K output | Do not use as default |
| `gpt-5.3-spark` | **Fast model**: quick analysis and rapid iteration | 400K input / 128K output | low-latency responses, high-throughput coding tasks |
...
**Default model rule**: Ryan explicitly set the Codex default to `gpt-5.5`. Do not change or downgrade this default unless Ryan explicitly says to.
...
Requires a Codex CLI build that supports `gpt-5.5`. The configured local default must remain `gpt-5.5`. Check version: `codex --version`
