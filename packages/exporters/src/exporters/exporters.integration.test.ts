...
describe("core exporters integration", () => {
  it("writes canonical JSON outputs for the TypeScript asset tree", () => {
    ...
    const pokemon = JSON.parse(
      fs.readFileSync(path.join(dataDir, "pokemon_data.json"), "utf8"),
    ) as Array<{
      id?: string;
      int_id?: number;
    ...
    const moves = JSON.parse(
      fs.readFileSync(path.join(dataDir, "moves_data.json"), "utf8"),
    ) as Record<string, { name?: string; type?: string; pp?: number }>;
    ...
    const learnsets = JSON.parse(
      fs.readFileSync(path.join(dataDir, "learnsets.json"), "utf8"),
    ...
    const levelUpMoves = JSON.parse(
      fs.readFileSync(path.join(dataDir, "level_up_moves.json"), "utf8"),
    ) as Record<string, Array<{ level: number; move: string }>>;
    ...
    const eggMoves = JSON.parse(
      fs.readFileSync(path.join(dataDir, "egg_moves.json"), "utf8"),
    ) as Record<string, string[]>;
    expect(eggMoves.TOTODILE).toEqual(expect.arrayContaining(["CRUNCH"]));

    const items = JSON.parse(
      fs.readFileSync(path.join(dataDir, "items.json"), "utf8"),
    ) as Array<{
      name?: string;
      script_name?: string;
      effect?: string;
    ...
    expect(fleeMons.buckets.always).toEqual(
      expect.arrayContaining(["RAIKOU", "ENTEI"]),
    );
    ...
    expect(marts.MART_CHERRYGROVE).toEqual([
      "POTION",
      "ANTIDOTE",
      "PARLYZ_HEAL",
      "AWAKENING",
    ...
    const pcStrings = JSON.parse(
      fs.readFileSync(path.join(dataDir, "pc_strings.json"), "utf8"),
    ) as Record<string, string>;
    ...
    const evolutions = JSON.parse(
      fs.readFileSync(path.join(dataDir, "evolutions.json"), "utf8"),
    ) as Array<{
      species?: string;
    }>;
    expect(evolutions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ species: "TOTODILE" }),
      ]),
    );
    ...
    const trainers = JSON.parse(
      fs.readFileSync(path.join(dataDir, "trainers.json"), "utf8"),
    ) as Array<{
      trainer_id?: string;
      base_reward?: number;
    ...
    expect(
      trainers.find((trainer) => trainer.trainer_id === "JACK1")?.base_reward,
    ).toBe(8);
    expect(
      trainers.every(
        (trainer) =>
          typeof trainer.base_reward === "number" && trainer.base_reward > 0,
      ),
    ).toBe(true);
    ...
    ) as Record<string, Array<{ script?: string; object_identifier?: string }>>;
    ...
    const pokedex = JSON.parse(
      fs.readFileSync(path.join(dataDir, "pokedex.json"), "utf8"),
    ) as Array<{
      species?: string;
      text?: string;
    }>;
    ...
    expect(pokegearLandmarks.map_to_landmark.PlayersHouse2F).toBe(
      "LANDMARK_NEW_BARK_TOWN",
    );
    expect(pokegearLandmarks.map_to_landmark.PalletTown).toBe(
      "LANDMARK_PALLET_TOWN",
    );

    const pokegearPaletteMap = JSON.parse(
      fs.readFileSync(
        path.join(dataDir, "pokegear_town_map_palette_map.json"),
        "utf8",
      ),
    ) as Record<string, string[]>;
    ...
    const contentPackIndex = JSON.parse(
      fs.readFileSync(
        path.join(dataDir, "content-packs", "index.json"),
        "utf8",
      ),
    ) as {
      packs: Array<{
    ...
    const corePack = contentPackIndex.packs.find(
      (pack) => pack.id === "core-modular",
    );
    const route29Pack = contentPackIndex.packs.find(
      (pack) => pack.id === "module-route-Route29",
    );
    ...
    expect(
      Object.values(corePack?.files ?? {}).every((files) => files.length === 0),
    ).toBe(true);
    ...
    expect(
      contentPackIndex.packs
        .filter((pack) => pack.id.startsWith("module-"))
        .every((pack) => pack.enabled === false),
    ).toBe(true);

    const generatedCorePack = JSON.parse(
      fs.readFileSync(
        path.join(dataDir, "content-packs", "core-modular.generated.json"),
        "utf8",
      ),
    ...
    expect(route29Map.Route29YoungsterText).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          command: "text",
          args: expect.stringContaining("Yo."),
        }),
        expect.objectContaining({
          command: "line",
          args: expect.stringContaining("#MON"),
        }),
      ]),
    );
    ...
    for (const category of requiredCoreCategories) {
      expect(generatedCorePack.files[category]?.length ?? 0).toBeGreaterThan(0);
    }

    for (const [category, files] of Object.entries(generatedCorePack.files)) {
      for (const relativePath of files) {
        ...
        if (category === "audio") {
          ...
          expect(entry.path.endsWith(".pcm")).toBe(true);
          expect(entry.source).toBe("pcm");
          expect(entry.pcm_format).toEqual({
            sample_rate_hz: 22_050,
            channels: 2,
            bits_per_sample: 16,
          });
          expect(entry.pcm_frame_count).toBeGreaterThan(0);
          const bytes = fs.readFileSync(path.join(dataDir, entry.path));
          expect(bytes.length).toBe(
            entry.pcm_frame_count *
              entry.pcm_format.channels *
              (entry.pcm_format.bits_per_sample / 8),
          );
          continue;
        }
        ...
        const firstPayloadEntry = () =>
          Object.values(payload)[0] as Record<string, unknown>;
        if (category === "pokemon") {
          ...
          expect(firstPayloadEntry()).toEqual(
            expect.objectContaining({ species: expect.any(String) }),
          );
          ...
          expect(
            Object.keys(payload).some((key) => key.endsWith("_MapScripts")),
          ).toBe(true);
          expect(
            Object.keys(payload).some((key) => key.endsWith("_MapEvents")),
          ).toBe(true);
          ...
          const [entry] = Object.values(payload) as Array<
          ...
          expect(firstPayloadEntry()).toEqual(
            expect.objectContaining({ map_name: expect.any(String) }),
          );
    ...
    for (const directory of ["music", "sfx", "cries"]) {
      ...
      for (const fileName of midiFiles) {
        expect(fs.readFileSync(path.join(audioDirectory, fileName)).subarray(0, 4))
    ...
    expect(
      fs.existsSync(
        path.resolve(assetRoot, "gfx", "tilesets", "whirlpool", "1.2bpp"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.resolve(assetRoot, "gfx", "font", "font_battle_extra.2bpp"),
      ),
    ).toBe(true);
    const fontExtra = fs.readFileSync(
      path.resolve(assetRoot, "gfx", "font", "font_extra.2bpp"),
    );
    const ellipsisTile = decode2bppTileLevels(fontExtra, 21);
    expect(ellipsisTile[0]).toBe(0);
    expect(ellipsisTile.filter((level) => level === 0).length).toBeGreaterThan(
      50,
    );
